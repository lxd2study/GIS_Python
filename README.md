...
面向 Landsat 8 影像预处理的本地化 Web 平台，集成单景预处理、批量流程编排、多影像在线检索下载与结果预览。系统同时支持 `L1` 传统预处理链和 `L2` 地表反射率直用分析，下载检索模块首期支持 Landsat 与 Sentinel-2，前后端一体运行。
...
| 影像下载检索 | STAC 检索 Landsat `L1/L2` 与 Sentinel-2 `L2A` 场景，支持地图框选 AOI、资产勾选、浏览器下载与服务端下载队列 |
...
4. 如需在线取数，先在影像下载页完成 AOI 检索与资产下载；如果访问 STAC 或下载资产较慢，可先在页面右上角配置 HTTP/HTTPS 代理。
