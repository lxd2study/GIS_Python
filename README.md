...
面向 Landsat 8 影像的本地化 Web 预处理平台，集成单景预处理、批量流程编排、Landsat 场景检索下载与结果预览。系统同时支持 `L1` 传统预处理链和 `L2` 地表反射率直用分析，前后端一体运行。
...
| Landsat 下载 | STAC 检索 `L1/L2` 场景、地图框选 AOI、资产勾选、浏览器下载与服务端下载队列 |
...
| Landsat 下载页 | 面向在线检索与下载，支持直接把下载场景送入后续处理 |
